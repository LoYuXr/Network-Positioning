...
window.draw = function(ls) {
    ...
    for (let i=0; i<ls.length; i++) {
        curve.push(new THREE.Vector2(ls[i][0], ls[i][1]));
    }
    curve.push(new THREE.Vector2(ls[0][0], ls[0][1]));
    curve = new THREE.SplineCurve(curve);
    ...
    function render() {
        ...
        // move agent
        const agentTime = time * 0.2;  // 0.05 is velocity
